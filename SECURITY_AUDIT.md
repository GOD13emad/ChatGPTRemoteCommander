...
PASS for the current v0.8.22 release candidate content and reachable branch/tag history checked locally after remote-ref synchronization.
...
- v0.8.22 prevents nested update generations from overwriting unresolved `route.previous` state. Both platform updaters gate cutover on existing-drain completion, and `router-switch.mjs` independently fails closed with `ROUTER_PREVIOUS_NOT_DRAINED`.
- v0.8.22 extends persistent-terminal protection to the Windows deferred-maintenance direct-stop path, so zero router inflight is not treated as permission to terminate a terminal-owning backend.
- v0.8.21 provides guarded GNOME/Wayland GUI support while retaining observe-only default and explicit-current-user takeover authorization.
...
- v0.8.18 detaches long-lived Windows staged backends from updater MCP stdio and adds a behavioral regression proving the parent pipe closes while the backend remains alive.
- v0.8.18 binds runtime server version to package/plugin release metadata with a regression, preventing updater/doctor authority drift.
- v0.8.17 closes the updater lock descriptor in promoted long-lived Linux backend/router children so lock lifetime is bounded to the updater process itself.\n- v0.8.16 restores bounded tunnel-client downloads and makes Linux lifecycle cleanup ownership-aware for routed processes; executable-mode and portable-runtime regressions prevent release/update drift.\n- v0.8.15 removes browser Fetch blocked-port semantics from the local stable-router test harness by using `node:http` for loopback test requests. This is test-infrastructure hardening only; router runtime code and security boundaries are unchanged.
