...
export const powerToolDefinitions = [
  ...
  { name: 'run_shell', description: 'Run a platform shell command (PowerShell on Windows, Bash on Linux) with timeout/output bounds. Explicit Power Mode only.', inputSchema: { type: 'object', properties: { command: { type: 'string' }, cwd: { type: 'string' }, timeoutMs: { type: 'integer', minimum: 1000 } }, required: ['command'], additionalProperties: false }, annotations: destructive },
  ...
  { name: 'start_terminal', description: 'Start a persistent platform terminal session (PowerShell on Windows, Bash on Linux) and optionally run an initial command.', inputSchema: { type: 'object', properties: { cwd: { type: 'string' }, command: { type: 'string' } }, additionalProperties: false }, annotations: destructive },
