...
import { canonicalizeRoots } from './security.mjs';
import { audit, listDirectory, readText, runProjectCommand, writeText } from './tools.mjs';
import { executePowerTool, powerToolDefinitions } from './power-tools.mjs';
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Run one allowlisted program through pwsh.exe in an allowed project directory.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  switch (name) {
    case 'system_status':
      ...
      return {
        ...
        allowedPrograms: config.allowedPrograms, powerMode: config.powerMode ?? { enabled: false }
      };
    case 'list_directory':
  ...
  try {
    if (!modern && message.method === 'initialize') {
      ...
      return { status: 200, body: rpcResult(message.id, {
        ...
        instructions: 'Operate only inside configured project roots.'
      }, false) };
    }
    if (modern && message.method === 'server/discover') {
      return { status: 200, body: rpcResult(message.id, {
        ...
        capabilities: { tools: {} },
        instructions: 'Operate only inside configured project roots. Prefer read-only inspection before mutation.',
        ttlMs: 300000,
        cacheScope: 'public'
      }, true) };
    }
