...
Worker reservations and provider calls consume the existing durable planner-call budget. A crash after reservation is not blindly retried. If a process exits after the journaled project import but before the worker receipt is closed, restart adopts that exact import only when the workflow receipt exists and the target bytes still match the recorded SHA-256; otherwise the run blocks for inspection. Pause/cancel/deadline checks remain authoritative while a worker provider call is active.
