...
Вставьте одну из команд в PowerShell. Standard безопасен по умолчанию; Power Mode включает полный доступ к файлам, Shell и процессам на доверенном ПК.
...
## v0.3: несколько компьютеров, несколько аккаунтов, параллельные чаты и без повторного ввода данных туннеля

- Один аккаунт ChatGPT может управлять несколькими компьютерами: установите проект на каждый компьютер и создайте отдельный Secure MCP Tunnel для каждого устройства.
- Несколько аккаунтов ChatGPT могут использовать один компьютер: зарегистрируйте каждый аккаунт с отдельным Tunnel Profile; порт проверки состояния выбирается автоматически.
- Несколько чатов могут одновременно обращаться к одному MCP; изменения одного и того же пути сериализуются для уменьшения конфликтов записи.

Однократная регистрация и автозапуск в Windows:

```powershell
.\enable-autostart.ps1 -Profile chatgpt-remote-commander
```

Однократная регистрация и автозапуск в Linux:

```bash
./enable-autostart-linux.sh --profile "$(hostname)"
```

После этого локальный supervisor автоматически запускает MCP и зарегистрированные туннели при входе в систему. Повторно вводить Tunnel ID, локальный адрес/порт или Runtime API Key не требуется.
