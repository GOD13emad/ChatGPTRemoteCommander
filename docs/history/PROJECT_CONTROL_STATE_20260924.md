...
Brain revision: R27
Status: HISTORICAL / v0.8.34 checkpoint
As of: 2026-09-24
...
- Windows live state: `default` and `secondary` both active on v0.8.31 / `8bd1883...`; both route files have `previous=null`; official updater returns `CURRENT`; Full Power remains explicit with all known capabilities enabled unless explicitly disabled.
...
- secondary canonical router: 127.0.0.1:47834 -> backend 48832.
- Tunnel profile chatgpt-remote-commander targets http://127.0.0.1:47831/mcp.
- Tunnel profile secondary targets http://127.0.0.1:47834/mcp.
...
8. Blue/green promotion of default and secondary — Completed: PROMOTED.
...
- Candidate-only updater on the validated Windows target: CANDIDATE_PASS for exactly default and secondary, both doctor/hardware/shadow/live-store PASS.
- Cutover committed both canonical routes to generation 3 / v0.8.6. A pre-cutover long-lived request on the old secondary backend did not drain within 60 seconds and produced `DRAIN_TIMEOUT` after commit. The old backend had no child workload; exact marker/listener ownership was verified, it was stopped narrowly, router inflight cleared, and the updater's explicit post-commit maintenance path completed.
...
- Tunnel observations: both `/readyz` endpoints are currently PASS. The default tunnel recovered from transient control-plane/Cloudflare HTTP 502 responses; secondary logged one startup OAuth-discovery metadata warning but remains ready.
...
- %LOCALAPPDATA%\ChatGPTRemoteCommander\routing\secondary.json
...
- Objective: close Linux-specific lifecycle/release gaps found on the real `Linux test host` deployment without changing Windows GUI/runtime semantics.
...
- Live `Linux test host`: BLOCKED BY ACCESS, not by release validation. Device is alive on LAN at 192.168.10.6 but tunnel-client is not polling; no SSH/RDP/VNC/WinRM/canonical MCP or bounded common remote-management port is reachable. Live runtime remains last-confirmed v0.8.13 until directly revalidated.
...
- Exact next action: execute the single recovery runner locally on `Linux test host` (or regain Remote Commander), then capture the generated Return and complete live promotion/second-update verification.
...
- Execution scope: local on `Linux test host` only; fail-closed if existing tunnel identity/credential is missing; no secret capture or credential deletion.
...
- Linux live state after v0.8.19: `Linux test host` runs exact commit `4538b1b...`, package/runtime 0.8.19, route `previous=null`, only the v0.8.19 release directory, updater lock FREE, user service active and canonical health/tunnel ready.
...
- Confirmed live topology: default canonical 47831 and isolated secondary canonical 47834; both Full Power, GUI enabled with explicit-current-request-only policy, independent tunnel profiles and route states.
- Confirmed blocker: default generation 19 retains previous v0.8.19 with persistent terminals, including an active scientific PowerShell workload. secondary has no previous route.
...
- Previous accepted state: `secondary` active v0.8.25 generation 22; default active v0.8.21 generation 19 with previous v0.8.19. v0.8.25 control plane active.
