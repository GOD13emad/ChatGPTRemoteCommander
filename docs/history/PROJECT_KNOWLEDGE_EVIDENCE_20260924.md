...
> Historical engineering record, archived 2026-09-24. Version, status and next-action statements below apply only to their original checkpoints. For current public status use [project state](../PROJECT_CONTROL_STATE.md). Local deployment identifiers have been generalized and private paths/process snapshots omitted; original receipts remain outside this public documentation. This cleanup does not purge Git history.
...
- Claim/Decision: Get-Targets recursively enumerated every instance.json below the instances root. Historical secondary backup records were therefore treated as live targets, producing repeated secondary candidates and eventually CANDIDATE_CLEANUP_OWNERSHIP_MISMATCH.
- Evidence/Source: failed v0.8.3 last-update state showed one default plus repeated secondary candidates; source inspection showed recursive instance.json discovery.
...
- Claim/Decision: Exactly two targets were discovered: default and secondary. Both passed hardware, doctor, shadow workflow store and live-store compatibility gates. Candidate processes were ownership-verified and stopped before cutover.
...
- secondary config SHA-256: 7d9d201378e42bb45a2a8d32a6a23f0b6dbf67a516458face284ba2232f2f22d.
- Route file SHA-256: default 973bedd39405f9ca23ac3964ca3551394bb0277096a44c65f8c39afd23a1795f; secondary 8510c30e4a5b414bb227235bb65897e7ea9446380f51e5d3dddbd8ca664b8765.
...
- Tunnel routing: YAML targets are 47831 for default and 47834 for secondary; tunnel-client processes remain running.
...
- Candidate-only validation returned CANDIDATE_PASS for exactly default and secondary.
...
- Active routes are generation 2:
  ...
  - secondary canonical 47834 -> backend 48834; config SHA-256 438453876b955316a7498e3312f66b515e7e8541bbdd8722986d900dcc4604db.
...
- Both canonical doctors PASS:
  ...
  - secondary 47834 -> 48834: v0.8.5, expected device match, 54 tools / 15 GUI tools, FULL_POWER, no doctor warning.
...
- Tunnel log observations: the default tunnel recovered from transient Cloudflare/control-plane HTTP 502 responses; secondary logged a startup OAuth-discovery metadata warning. Current readiness is PASS for both, so these are non-blocking observations rather than active failures.
...
- Poststate: both Remote Commander workflows checkpointed current v0.8.5 evidence and then were explicitly paused. Both now report revision 4, lifecycle `WAITING`, schedulerEnabled=false in `workflow_list`; default scheduler pending count fell to 1 (the untouched thesis workflow) and secondary pending count fell to 0. Their next action points to the external billing gate and a separate future supply-chain hardening change set.
...
- Candidate-only updater validation returned `CANDIDATE_PASS` for exactly `default` and `secondary`. Both profiles passed hardware, doctor, shadow workflow-store and live-store compatibility gates.
- Promotion independently reran check/test/security/native-GUI gates and committed both canonical routes to generation 3 / v0.8.6:
  ...
  - secondary 47834 -> 48832, config SHA-256 `a3d2f2da1e23e5e97b3047fa91418269f74f2cf98c8d5884bf3753487ee47764`.
- Post-commit drain incident: the old secondary backend on 48834 retained one router-counted in-flight request past the 60-second drain budget, producing `DRAIN_TIMEOUT profile=secondary` and `PROMOTED_MAINTENANCE_REQUIRED` after routes were already committed.
...
- Date/Context: 2026-09-21, follow-up to the v0.8.6 `DRAIN_TIMEOUT profile=secondary` incident.
...
- Date/Context: 2026-09-21, exact v0.8.10 commit `076e22c32c589a4ddfb04651e2ae774a2dba8c0a` after candidate-only PASS and blue/green cutover on Windows test host.
...
- New failures carried forward from exact-public v0.8.12 validation: `GUI_NATIVE_BUSY` caused by updater use of shared interactive `gui_status`; a failed secondary candidate remained listening because Windows cleanup tracked only candidates that had completed all checks; the next run failed workflow-shadow copy.
...
- v0.8.14 full source validation PASS on Windows test host: `npm run check`, `npm test`, `npm run audit`, Windows native no-input GUI self-test and Git-for-Windows Bash syntax validation all returned zero.
...
- Date/Context: 2026-09-21, live audit of the newly enrolled Ubuntu 24.04-class target `Linux test host` plus exact source authority at origin/main v0.8.15.
...
- Provenance: source paths `autostart-linux.sh`, `enable-autostart-linux.sh`, `disable-autostart-linux.sh`, `connect-chatgpt-account.sh`, `install.sh`, `test/source-integrity.mjs`; live Ubuntu canonical route and systemd evidence captured through Remote Commander on `Linux test host`.
...
- Provenance: live `/proc/<pid>/fd/9`, `fuser`, canonical route/update logs on `Linux test host`; source `auto-update-linux.sh`, `test/auto-update-contract.test.mjs`.
...
- Live target gate: `Linux test host` is reachable on LAN at 192.168.10.6 and replies to ping, but its OpenAI tunnel-client has not been seen for >300 seconds. SSH, RDP/VNC/WinRM, canonical MCP/tunnel ports and a bounded set of common remote-management ports are closed from the trusted Windows host. Therefore there is presently no authorized remote execution path to restart the local user service.
- Validation status: product/release PASS; live deployment on `Linux test host` remains UNVERIFIED/OPEN. No claim of live v0.8.17 promotion is made.
...
- Exact next action: regain one local execution path on `Linux test host`, release the legacy inherited updater lock by ownership-safe service/process shutdown, run the exact v0.8.17 installer/update, then require canonical route/tunnel health plus a second updater run proving `AUTO_UPDATE_ALREADY_RUNNING` is gone.
...
- Date/Context: 2026-09-21, created only because `Linux test host` is LAN-reachable but exposes no authorized remote execution channel and its OpenAI tunnel-client is not polling.
...
- Linux live evidence: `Linux test host` runs exact v0.8.19 commit/package/runtime; route generation 5 has `previous=null`; only release `v0.8.19-4538b1b7931c` remains; updater lock is FREE; systemd user service is active; canonical health returns 0.8.19. Log records `AUTO_UPDATE_PASS version=0.8.19 commit=4538b1b...`.
...
- Date/Context: 2026-09-22 on Linux test host, after remote authority unexpectedly advanced from v0.8.18 to public v0.8.19/v0.8.20 during local GUI engineering.
...
- Windows preservation evidence: default route remains generation 19, active v0.8.21 and previous v0.8.19; secondary remains generation 20, active v0.8.21 with `previous=null`. No candidate listeners remained on staged ports 48832/48835. Supervisor PID remained 30584. Existing old backend has live persistent terminals and a router-recorded non-cancellable `run_shell` request; it was not force-stopped.
...
- Date/Context: 2026-09-22, live audit on Windows test host with two ChatGPT tunnel profiles.
...
- Evidence/Source: live route/health/process/tunnel state; default route generation 19 active v0.8.21 + previous v0.8.19; secondary generation 20 active v0.8.21 + previous=null; v0.8.24 last-update.json showed both candidates passed hardware/doctor/shadow/live-store gates but global status was BLOCKED_EXISTING_DRAIN.
...
- Provenance: exact local clean checkouts on Windows test host and Linux test host; live system_status/gui/helper/MCP probes; public GitHub release metadata; commit `cf0e6770d31a904f7ed7cf4eb799476bdfa731ea`.
...
- Windows verification: live `system_status.version=0.8.31`; Full Power; active/queued operations zero at audit; `default` and `secondary` routes both active at `8bd1883...` with `previous=null`; updater result `CURRENT v0.8.31`; live GUI `windows-user32-gdi` available/enabled/idle/unleased.
...
- Installer isolation proof: second exact-tag custom `-NoStartServer` acceptance returned exitCode=0 and `INSTALL_PASS`; custom install HEAD/version were `8bd1883...`/0.8.31; production Windows app HEAD plus SHA-256 of `routing/default.json`, `routing/secondary.json`, `retained-backends.json`, and `last-update.json` were exactly unchanged before/after.
...
- Provenance: Windows test host live runtime/process/routing evidence; Linux test host live runtime/workflow/native-GUI/MCP catalog evidence; GitHub release/tag/main authority; exact commit `8bd18836882e5f4a6dd2497142488ac1af93dd70`.
...
## E062 — adaptive engine integration and restricted-path diagnosis

- Context: milestone 4A development from `67d0153`; execution environment changed to restricted filesystem access during this turn.
- Confirmed failure: both the previous baseline demo and the new adaptive demo failed at standard `writeText` before mutation. Direct diagnosis on an owned temporary fixture identified `EPERM` while canonicalizing the ancestor user-profile directory inside the shared write guard; a direct write/read in the exact authorized fixture succeeded.
- Root cause: canonicalizing every ancestor demanded unrelated ancestor metadata even when the permitted descendant's complete canonical path could be resolved. This was separate from adaptive planning/team orchestration; the adaptive demo had already committed its prerequisite insertion and source-read receipt.
- The same ancestor-realpath requirement also existed in verifier inspection/revalidation. Both paths were corrected: retain per-component lstat/link/regular-file/hardlink and identity checks, and validate the entire canonical path at the deepest existing permitted target/parent. No EPERM is ignored and no unverified canonical path is accepted. Independent review confirmed the invariant before the change.
- Focused qualification after correction: 55 PASS / 1 file-symlink privilege SKIP across guard, verifier, adaptive engine and integration suites. Existing junction/hardlink and identity-race checks passed. Five new guard cases cover authorized writes/backups/hash checks, missing parent levels, junctions, hardlinks and identity replacement.
