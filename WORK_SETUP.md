...
Work must use the coordination protocol: `gui_status → gui_session_begin → gui_screenshot(lease) → one action(lease+frame) → gui_screenshot(lease) → verify → gui_session_end`. `gui_session_begin` is observe-only by default. Mouse/keyboard/scroll/focus actions require `mode="takeover"` plus `explicitUserAuthorization`, and Work may request that mode only when the user's current request explicitly asks it to take/control/interact with the desktop UI. Work must not decide on its own to take the user's mouse or foreground browser/window; prefer headless, shell/API, filesystem, or isolated/background alternatives. Frames are short-lived and single-use; never replay an action after a timeout or uncertain native result. If another chat holds the lease, report the busy state rather than stealing or polling aggressively.
