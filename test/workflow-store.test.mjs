...
test('path traversal and out-of-scope memory store rejected',()=>{const f=fixture();try{f.create();assert.throws(()=>f.s.evidence(f.root,['../outside']),/OUT_OF_SCOPE/);assert.throws(()=>new WorkflowStore({...f.options,directory:path.join(os.tmpdir(),'rc-other')}),/OUT_OF_SCOPE/);}finally{f.dispose();}});
