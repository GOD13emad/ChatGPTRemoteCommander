...
async function withMockServer({ version = '0.7.3', deviceName = 'device-a', tools, configSha256 = null }, fn) {
...
test('runDoctor passes healthy matching server and config', async () => {
  ...
  try {
    ...
    await withMockServer({ configSha256: sha }, async (endpoint) => {
      const report = await runDoctor({ endpoint, expectedVersion: '0.7.3', expectedDevice: 'device-a', configPath, timeoutMs: 3000 });
...
test('runDoctor reports version drift', async () => {
  await withMockServer({ version: '0.5.1' }, async (endpoint) => {
    const report = await runDoctor({ endpoint, expectedVersion: '0.7.3', timeoutMs: 3000 });
