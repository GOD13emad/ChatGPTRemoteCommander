...
test('HTTP retry hardening rejects long sync work before effect and bounds oversized responses', async () => {
  ...
  try {
    ...
    const direct = await rawPost(port, 1, 'run_project_command', { program: 'node', args: [fixture], cwd: canonicalDataRoot, timeoutMs: 20001 });
    ...
    assert.match(direct.body.result.content[0].text, /20000|maximum/);
    await wait(1200);
