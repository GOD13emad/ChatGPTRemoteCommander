...
test('catalog has eleven unique tools and conservative action annotation',()=>{
 assert.equal(WORKFLOW_TOOL_DEFINITIONS.length,11);assert.equal(new Set(WORKFLOW_TOOL_DEFINITIONS.map(d=>d.name)).size,11);
...
test('unsafe host policy rejected at initialization',()=>{const f=fixture();try{assert.throws(()=>createWorkflowTools({...f.base,config:{durableWorkflows:{enabled:true,directory:path.join(f.root,'other'),executionTools:['run_shell']}}}),/INVALID_TOOL_POLICY/);}finally{f.dispose();}});
