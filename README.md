...
[Documentation index](docs/README.md) · [Published baseline: v0.8.41](docs/RELEASE_0.8.41.md) · [Current candidate: v0.9.0](docs/RELEASE_0.9.0.md) · [Project status](docs/PROJECT_CONTROL_STATE.md) · [Development roadmap](docs/PROJECT_ENGINE_ROADMAP.md)
...
In v0.9.0, Full Power installations also converge the bounded Project Engine runner across qualified Windows/Linux deployments when the already-qualified local Codex provider is present; Standard authority and explicit opt-outs remain fail-closed. In v0.8.35, [durable input requests](docs/PROJECT_ENGINE_DECISIONS.md) let an enrolled project ask for a decision and resume from an explicit answer without resetting its budgets or changing its acceptance criteria.
...
v0.8.41 carries the hardened background-browser and durable-command layers from the unpublished v0.8.39/v0.8.40 candidates and adds transport-deadline/response-budget hardening. It drives an owned headless Chromium session through CDP, using a Commander-only persistent or isolated profile. It does **not** move the user's mouse, change foreground focus, type into the user's browser window, reuse the user's normal browser profile, or expose cookies/password-store contents.
