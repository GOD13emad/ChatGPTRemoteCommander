...
v0.8.7 keeps Capability Profile v2, Durable Workflows R2 and candidate-first automatic updates, and adds a persistent bounded Windows GUI helper plus safer post-cutover drain recovery. Repeated GUI reads no longer start and recompile a fresh PowerShell helper each time; on the validated Windows target, warm status/window/screenshot reads dropped from roughly 1.7–2.2 seconds to tens of milliseconds for status/window reads and about 80–90 ms for common screenshots while preserving lease/frame/foreground/emergency-stop rules. Explicit Full Power remains an all-on authority class. Windows and Linux validate releases side-by-side, switch stable loopback routers only after gates pass, defer unsafe old-backend drains instead of blindly killing unknown work, and automatically finish schema/supervisor/release maintenance once the old work is gone.
