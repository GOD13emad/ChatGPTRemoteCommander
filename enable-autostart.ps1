...
function Find-TunnelExe {
  ...
  if (-not (Test-Path -LiteralPath $stateFile)) { throw 'Pinned tunnel-client state is missing. Re-run install.ps1.' }
  $state = Get-Content -LiteralPath $stateFile -Raw | ConvertFrom-Json
  $exe = [IO.Path]::GetFullPath([string]$state.path)
  if (-not (Test-Path -LiteralPath $exe -PathType Leaf)) { throw 'Pinned tunnel-client executable is missing. Re-run install.ps1.' }
  $actual = (Get-FileHash -LiteralPath $exe -Algorithm SHA256).Hash.ToLowerInvariant()
  if ($actual -ne [string]$state.sha256) { throw 'Pinned tunnel-client SHA256 mismatch. Re-run install.ps1.' }
  return $exe
}
...
try {
  ...
  $exePath = [IO.Path]::GetFullPath($TunnelExe)
  $runningProfile = Get-CimInstance Win32_Process -Filter "Name='tunnel-client.exe'" -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      $_.CommandLine -match "--profile(?:=|\s+)$profilePattern(?:\s|$)"
    } |
...
Write-Host 'Windows logon autostart is enabled. No Tunnel ID, IP/port, or Runtime API key is required on later logons.'
 -or $Profile -match '\.\.' -or $Profile -in @('.','..')) {
...
Write-Host 'Windows logon autostart is enabled. No Tunnel ID, IP/port, or Runtime API key is required on later logons.'
  ...
  if (-not $tm.Success -or -not $hm.Success) { throw 'Existing tunnel profile is missing tunnel_id or loopback health port.' }
  $existingTunnelId = $tm.Groups[1].Value
  $existingHealthPort = [int]$hm.Groups[1].Value
  if ($TunnelId -and $TunnelId -ne $existingTunnelId) { throw 'TunnelId does not match the existing profile; use a different profile name.' }
  if ($HealthPort -ne 0 -and $HealthPort -ne $existingHealthPort) { throw 'HealthPort does not match the existing profile; use a different profile name.' }
